...
import PicomatchM from "../../../../../../cdn.jsdelivr.net/npm/picomatch@2.2.1/+esm.js";
...
const globCharsReS: RegexString = globChars.map((c) => `\\${c}`).join(
  "|",
) as RegexString;
// const pathSepRe = /[\\/]/;
// const sep = $path.sep;
...
const nonGlobQSepReS: RegexString =
  `(?:(?!${globCharsReS}|${QReS}|${pathSepReS}).)` as RegexString;
...
const cNonANonQNonWSReS: RegexString =
  `(?:(?![$]'|${QReS}|\\s).)` as RegexString;
...
const WordRxs = {
  ...
  quoteBasic: new RegExp(
    `^((?:${DQStringReS}|${SQStringStrictReS}))(.*?$)`,
    "msu",
  ),
...
export function shiftCLTextWord(s: string, options: {
  autoQuote: boolean;
} = { autoQuote: true }): [
  string,
  string,
...
export function wordSplitCLTextByShift(s: string, options: {
  autoQuote: boolean;
} = { autoQuote: true }): Array<string> {
  // parse string into tokens separated by unquoted-whitespace
  // * supports both single and double quotes
  // * no character escape sequences are recognized
  // * unbalanced quotes are allowed (parsed as if EOL is a completing quote)
...
export function wordSplitCLText(s: string, options: {
  autoQuote: boolean;
} = { autoQuote: true }): Array<string> {
    ...
    (escapeString) => {
      ...
      if (escapeCodeType === "u" || escapeCodeType === "U") {
        ...
        const codesRaw = codesText.map((codeText) => {
          ...
          return parseInt(
            codeText[0] === "x" ? codeText.slice(1) : codeText,
            parseBase,
          );
        });
    ...
    if (m) {
      ...
      if (matchStr.length > 0) {
        if (matchStr[0] === DQ || matchStr[0] === SQ) {
          ...
          matchStr.length > 1 && matchStr[0] === "$" && matchStr[1] === SQ
        ) {
          // $'...'
...
export async function* globExpandIter(
  ...
  const caseSensitive = !isWinOS;
  const globHasTrailingSep = glob.match(
    new RegExp(`${$path.SEP_PATTERN.source}$`),
  );
  const globWithoutTrailingSep = glob.replace(
    new RegExp(`${$path.SEP_PATTERN.source}$`),
    "",
  );
  ...
  if (parsed.glob.length > 0) {
    ...
    if (await $fs.exists(resolvedPrefix)) {
      ...
      const globEscapedPrefix = escapeRegExp(
        resolvedPrefix +
          (resolvedHasTrailingSep || initialGlobstar ? "" : $path.SEP),
      ).replace(/\\\\|\//g, "[\\\\/]");
...
export function* globExpandIterSync(
  ...
  if (parsed.glob.length > 0) {
    ...
    if ($fs.existsSync(resolvedPrefix)) {
      ...
      // some paths are resolved to paths with trailing separators (eg, root or network paths) and other are not
      // const trailingSep = globEscapedPrefix.endsWith('[\\\\/]');
      const maxDepth = parsed.globScanTokens.reduce((acc: number, value: {
        value: string;
        depth: number;
        isGlob: boolean;
...
export function globToReS(s: string) {
  const caseSensitive = !isWinOS;
  const tokenRe = new RegExp(
    `^((?:${DQStringReS}|${SQStringStrictReS}|${cNonDQReS}+))(.*?$)`,
    "",
  );
  let text = "";
  while (s) {
    ...
    if (m) {
      ...
      if (matchStr.length > 0) {
        ...
        if (
          firstChar === DQ ||
          (matchStr.length > 1 && firstChar === SQ &&
...
export async function argsAsync(
  ...
  const raw = arr.length && idx > 0 && idx < arr.length
    ? arr.slice(idx + 1)
    : [];
...
export async function* argsItAsync(
  ...
  while (argsText) {
    ...
    if (argText === endExpansionToken) {
      continueExpansions = false;
    }
          ...
          tailOfArgExpansion: [
            ...(!next.done
              ? [
                (async function* () {
                  yield next.value;
                  for await (const e of argExpansion) {
                    yield e;
                  }
                })(),
              ]
